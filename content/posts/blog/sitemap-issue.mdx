...
> 혹시나 누군가에겐 해결책이 될 수 있으니 제가 시도해본 방법들을 남겨놓겠습니다.
>
> - `sitemap.xml/?test` 처럼 URL의 뒤에 `?`나 `/`를 붙여서 새로 시도해보세요.
> - `sitemap.xml` 파일을 `sitemap`으로 변경해보세요.
> - `sitemap.xml` 대신 `rss.xml`이나 `atom.xml` 파일을 만들어보세요.
> - `robots.txt` 파일에 `Sitemap: https://{도메인}/sitemap.xml` 를 추가해보세요.
> - 사이트 속성 말고 도메인 속성으로 등록해서 시도해보세요.
> - 이미 도메인 속성이라면 `https://{도메인}/sitemap.xml` 처럼 전체 경로를 입력해보세요.
...
현재 실제로 사이트맵이 등록되고 페이지가 제대로 색인되는 중입니다.
글이 매우 짧고 간단하지만 누군가에게 도움이 되길 바라는 마음으로 글을 납깁니다.
