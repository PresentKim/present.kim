...
thumbnail: '/assets/posts/php/php-talkative-01-intro/thumbnail.png'
date: 2024-04-03 10:12:00
---

![P-H-P-H-P](/assets/posts/php/p-h-p-h-p.png)

<br />

# 🚩 개요

제가 [간단하게 알아보는 PHP의 역사](/posts/php/php-history)를 작성한 지 반년이 지났습니다.
...
**![The elePHPant](/assets/posts/php/php-talkative-01-intro/mascot.png) _The elePHPant_**
...
![How to design elePHPant](/assets/posts/php/php-talkative-01-intro/mascot_from_php.png)
...
[![W3Techs : 2022.01 ~ 2023.01 서버 사이드 언어 점유율](/assets/posts/php/php-talkative-01-intro/w3techs-server-side.png)
_📊W3Techs : 2022.01 ~ 2023.01 서버 사이드 언어
점유율_](https://w3techs.com/technologies/history_overview/programming_language)

[![stackoverflow : 2022 스크립트·마크업 언어 선호도·기피도](/assets/posts/php/php-talkative-01-intro/stackoverflow-loved-dreaded-survey.png)
...
PHP 파일은 아래와 같이 `<?php ... ?>{:php}` 태그 안에 PHP 코드가 삽입되고,
이 외의 부분은 `echo{:php}` 키워드로 출력된 것과 동일하게 처리됩니다.

설정을 통해 `<?{:php}`(SGML) 나 `<%{:php}`(ASP)로 시작하는 태그를 사용할 수도 있습니다.
하지만 왠만한 경우엔 이식성 문제로 인해 `<?php{:php}`를 사용하는 것이 좋습니다.
...
```text title="출력 결과" showLineNumbers
<html>
    <body>
        Hello World!
    </body>
...
하지만, 스크립트 파일로서 사용될 때는 오히려 `?>{:php}` 태그를 사용하지 않는 것이 권장됩니다.
이유는 `?>{:php}`로 닫힌 이후에 공백이나 줄바꿈이 있으면 의도치 않은 출력이 생길 수 있기 때문입니다.

또한 PHP에는 `<?={:php}`라는 단축 표현이 있습니다. 이 표현은 `<?php echo{:php}`를 대신하여 사용할 수 있습니다.
또한 PHP의 모든 명령문은 C언어처럼 세미콜론(`;{:php}`)으로 끝나야 합니다.
하지만 `?>{:php}`태그로 닫히면 세미콜론을 생략해도 됩니다.
...
> **![facebook meme](/assets/posts/php/php-talkative-01-intro/facebook-meme.png)**
...
이 글은 PHP의 역사라고 정리하기에는 부족하지만, PHP에 관련된 이야기를 기록하고 싶어 작성한 글입니다.
PHP를 사용하면서 가졌던 흥미나 관심으로 알게된 내용들을 정리하는 잡담이므로 여기에서 마무리하겠습니다! 감사합니다.
