...
thumbnail: '/assets/posts/php/php-history/thumbnail.png'
date: 2023-10-24 12:43:00
---

## _**![PHP Logo](/assets/posts/php/php-history/title.png)**_
...
이 포스트에서는 28년이라는 긴 세월 동안 수많은 개발자들의 기여로 발전해온 PHP의 역사를 정리해보도록 하겠습니다.

> 현재 PHP는 `The PHP Group`에서 개발 및 관리를 맡고 있으며,
> 공식 웹사이트인 [php.net](http://www.php.net)을 통해 문서를 제공하고 있습니다.
> 이 곳에서는 3.0 이상의 버전만 관리하기 때문에 초기 버전인 1.0과 2.0 버전은 [PHP 박물관](https://museum.php.net)에서 확인하실 수 있습니다.
...
1994년, 덴마크계 캐나다인 프로그래머 Rasmus Lerdorf는 자신의 온라인 이력서를 방문하는 사람들을 추적하기 위한 간단한 도구가 필요했습니다.
이를 위해 Perl로 CGI 프로그램을 작성했는데,
이것이 PHP의 시작이 되었습니다.

웹서버의 과부하 문제와 개발의 복잡성을 해결하기 위해 C언어로 Perl 래퍼를 다시 작성하면서,
점차 기능이 확장되었고 주변 개발자들의 관심을 끌기 시작했습니다.
그들의 피드백과 기여를 받아들이며 도구는 계속 발전했고,
마침내 1995년 `Personal Home Page Tools`라는 이름으로 소스코드가 공개되었습니다.
...
PHP의 진화는 계속되었습니다.
Rasmus는 PHP Tools를 개발하면서 동시에 데이터베이스와의 더 나은 연동 방법을 모색했고,
이는 `Form Interpreter`라는 새로운 도구의 탄생으로 이어졌습니다.
이 도구는 SQL 쿼리를 파싱하고 HTML 폼과 테이블을 쉽게 생성할 수 있게 해주었습니다.

이 버전부터 프로그래밍 언어로 발전한 `PHP/FI`는 간단한 스크립트를 직접 HTML 파일에 배치할 수 있어,
C와 Perl 언어로 CGI를 사용해 복잡하게 웹페이지를 작성하던 기존 방식에 비해 매우 쉬운 편이였습니다.
또한, 수많은 CGI 도구를 사용할 때와 달리 오버헤드가 많이 제거되어 웹페이지의 성능이 개선되었고,
기존 개발자가 쉽게 배울 수 있도록 의도적으로 C와 Perl의 구조와 유사하게 설계되었습니다.
...
PHP 3.0에 이르러 객체 지향 프로그래밍의 지원과 강력하고 일관화된 언어 구문으로 더욱 개선되었고,
이에 더해 강력한 확장성으로 수많은 모듈 개발자들의 유입을 불러왔습니다.
...
Andi Gutmans와 Zeev Suraski는 본인들의 이름 _Zeev_ 와 _Andi_ 로부터 따온 `Zend Engine`이라는 이름의 새로운 엔진을 만들어 냈고, 성능과 확장성을 크게 개선했했습니다.

더 많은 웹 서버, HTTP 세션, 출력 버퍼링, 사용자 입력을 처리하는 보다 안전한 방법 및
몇 가지 새로운 언어 구조에 대한 지원과 같은 기타 주요 기능이 포함된 4.0 버전은 꾸준히 업데이트되어,
...
- 주요 사항: 네임스페이스(Namespace), 코드 조각(trait), 삼항 연산자(`?:`), 제네레이터(Generators), 클로저(Native closures), 단축 배열 구문(`[]`), 상수(const), 가변 길이 인수, PDO(PHP Data Object), JSON 함수, 내장 개발 서버, 가비지 컬렉션(Garbage Collection), Phar(php 묶음파일), SQLite3, ...

후속 버전인 PHP6의 개발이 2005년부터 시작되어 유니코드를 기반으로 작동하도록 설계하였는데,
이때 `UTF-16`를 차기 유니코드 인코딩으로 채택해 개발하는 사이,
...
문제는 이렇게 추가된 기능들을 PHP6이라고 지칭하는 많은 자료들이 존재하게 되었기 때문에,
PHP6이라는 버전을 사용하면 큰 혼동을 야기할 수 있다는 우려가 많았습니다.
...
PHP 8.0은 현대적인 프로그래밍 언어로서의 완성도를 한층 더 높인 버전입니다.
PHP7에서부터 준비해 온 `JIT`(Just-In-Time) 컴파일러의 도입으로 성능이 획기적으로 개선되었으며,
타입 시스템의 강화와 함께 더욱 안정적인 코드 작성이 가능해졌습니다.

특히 주목할 만한 새로운 기능들은 다음과 같습니다:

- 타입 안정성을 높여주는 읽기 전용 프로퍼티(readonly)
- 견고한 타입 시스템을 위한 열거형(Enum) 지원
- 더 간결하고 안전한 조건문 작성을 위한 `match` 표현식
- 메서드 체이닝을 안전하게 해주는 널 세이프 연산자(`?->`)
- Java의 어노테이션과 유사한 `Attributes`
